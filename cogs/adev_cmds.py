...
class DevCommands(commands.Cog, name='Dev', command_attrs=dict(hidden=True, add_check=is_dev)):
	...
	async def add_admin(self, ctx: discord.ext.commands.Context, u_id: str):
		...
		await ctx.message.channel.send(f'User with ID {u_id} has been added to the admin list.', delete_after = ctx.bot.del_after)
	...
	async def remove_admin(self, ctx: discord.ext.commands.Context, u_id: str):
		...
		await ctx.message.channel.send(f'User with ID {u_id} has been removed from the admin list.', delete_after = ctx.bot.del_after)
