...
from utils.discord_utils import get_member_by_id
...
async def rek_random_in_channel(ctx: CContext, channel: discord.VoiceChannel | discord.StageChannel):
    ...
    member: discord.Member | None = await get_member_by_id(to_rek, ctx.bot)
    if member is None:
...
async def last_log(ctx: commands.Context, anonymous: bool = False) -> None:
    mod_log_channel = await discord_utils.get_channel_by_id(1329367677940006952, ctx.bot)  # Channel where the carlbot logs are sent
    pub_logs_channel = await discord_utils.get_channel_by_id(1345300442376310885, ctx.bot)  # Public logs channel (#guillotine)
    
    if mod_log_channel is None or pub_logs_channel is None:
        await ctx.send("Mod log channel or public logs channel not found.")
        return
    ...
    if last_mod_log_message.content == "Posted":
        await ctx.send("This log has already been sent to the public logs channel.")
        return
    
    embed = last_mod_log_message.embeds[0]
    if embed is None or embed.title is None or embed.description is None:
        await ctx.send("No embed found in the last mod log message.")
        return
