	...
	async def nasa_pic(self, ctx: discord.ext.commands.Context):
		if os.path.exists(f'nasa/nasa_pic_{ctx.bot.today}.jpg'):
			...
			await ctx.message.channel.send(
				file=discord.File(f'nasa/nasa_pic_{ctx.bot.today}.jpg', filename=f'nasa_pic_{ctx.bot.today}.jpg'))
			await ctx.message.channel.send(f'**Explanation:** {ctx.bot.nasa_data["explanation"]}')
			return
