...
async def got_shot(ctx: commands.Context, profile: profile_type, target: discord.Member, target_profile: profile_type):
	...
	if profile['wallet'] >= amount:
		...
		curr_utils.set_debt(ctx.author, profile['debt'])
		added_to_debt = True
...
class CrimeCog(commands.Cog, name="Crime"):
	...
	async def rob_cmd(self, ctx: commands.Context, target: discord.Member) -> None:
		...
		if not curr_utils.user_has_gun(profile) and not curr_utils.user_has_gun(target_profile):
			...
			print("User has a gun, target has no gun")
